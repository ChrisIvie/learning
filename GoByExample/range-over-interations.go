...
// In that example we had an Allelments method that returned a slice of all elements in the list.
...
func (lst *List[T]) All() iter.Seq[T] {
    return func(yield func(T) bool) {
        ...
        // The iterator function as a parameter, called yield by convention (but the name can be arbitrary.)
...
// Here's a function returning an iterator over Fibonacci numbers: it keeps running as long as yield keep returning true. 
...
func main() {
    ...
    for n := range genFib() {
        ...
        // Once the loop hits break or early return, they yield function passed to the iterator will return false. 
        if n >= 10 {
            break
        }
        fmt.Println(n)
